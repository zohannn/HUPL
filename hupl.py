...
def NNopt_loss_function_err(X_df,costs_df,W):
  """
...
def NNopt_loss_function_err_sigma(X_df,costs_df,n_D,W,r):
  """
  ...
  Z = 0
  n_Dx = 1
  mn = n_D # number of nearest neighbors for sparsity
  W2 = np.square(W)
  dm = []
  xdiff2_m = []
  sparsity = True
  dm_sorted_df = pd.DataFrame()
  for jj in range(n_samples):
    xj = xj_df.iloc[jj]
    xi = xi_df.iloc[jj]
    cc = costs_df.iloc[jj, 0]  # total cost
    if (sparsity):
        for ii in range(n_D):
            xii = xi_df.iloc[ii]
        ...
        xdiff2_m.clear()
        sparsity = False
    sigma = r * (1 / mn) * np.sum(dm_sorted_df.iloc[0:mn, 0])
    #sigma = r * (dm_sorted_df.iloc[0,0])
    Z += k_sim_sigma(xj, xi, W, sigma)
    E1 += (k_sim_sigma(xj, xi, W, sigma)) * cc
    if ((jj+1) != n_samples):
        xjj = xj_df.iloc[jj+1]
        if(np.sqrt(np.sum(np.square(np.array(xj) - np.array(xjj)))) > 0.001):
            # the next row contains a different sample of the dataset
...
def NNopt_loss_function_err_sigma_1(X_df,costs_df,n_D,M,W,r):
  """
  ...
  for jj in range(n_Dx):
    ...
    sigma = r * (1 / mm) * np.sum(dm_sorted_df.iloc[0:mm, 0])

    Z = 0
    for ii in range(n_D):
        xii = xi_df.iloc[ii]
        Z += k_sim_sigma(xj, xii, W, sigma)
    E = 0
    if ((np.abs(Z)) < 1e-60):
        reg = 1e+60
    else:
    ...
    for ii in range(n_D):
        ...
        E += cc * reg * (k_sim_sigma(xj, xii, W, sigma))

    L += E
  return (L/n_Dx)

def NNopt_loss_function_err_sigma_2(X_df,costs_df,n_D,M,W):
  """
  ...
  for jj in range(n_Dx):
    ...
    dm_sorted_df = dm_df.sort_values(by=['distance_m'], ascending=True)
    dm_sorted_df = dm_sorted_df.reset_index(drop=True)
    dm_mean = (1 / mm) * np.sum(dm_sorted_df.iloc[0:mm, 0])
    sigma = r * dm_mean
    ...
    Z = np.sum(k_sim_sorted_df.iloc[0:nn,0])
    if ((np.abs(Z)) < 1e-60):
        reg = 1e+60
    else:
        reg = (1 / Z)
    ...
    E = np.sum(E_sorted_df.iloc[0:nn,1])

    L += E
  return (L/n_Dx)

def NNopt_loss_function_err_sigma_3(X_df,costs_df,n_D,M,W,r):
  """
...
def NNopt_loss_function_der_err_sigma_3(X_df,costs_df,n_D,M,W,r):
  """
  ...
  for jj in range(n_Dx):
    ...
    for ii in range(n_D):
        xii = xi_df.iloc[ii]
        cc = costs_df.iloc[(jj * n_D)+ii, 1]  # mean derivative of the total cost
        E_vect.append(cc * reg * (k_sim_sigma(xj, xii, W, sigma)))
    E_df = pd.DataFrame({'k_sim': k_sim_vect,'E_value': E_vect})
    E_sorted_df = E_df.sort_values(by=['k_sim'], ascending=False)
    E_sorted_df = E_sorted_df.reset_index(drop=True)
...
def NNopt_loss_function_iter_sigma_3(X_df,costs_df,n_D,M,W,r):
  """
  ...
  for jj in range(n_Dx):
    ...
    for ii in range(n_D):
        xii = xi_df.iloc[ii]
        cc = costs_df.iloc[(jj * n_D)+ii, 2]  # iterations
        E_vect.append(cc * reg * (k_sim_sigma(xj, xii, W, sigma)))
    E_df = pd.DataFrame({'k_sim': k_sim_vect,'E_value': E_vect})
    E_sorted_df = E_df.sort_values(by=['k_sim'], ascending=False)
    E_sorted_df = E_sorted_df.reset_index(drop=True)
...
def NNopt_loss_function_der_err(X_df,costs_df,W):
  '''
  ...
  for ii in range(n_samples):
    ...
    xii = xi_df.iloc[ii]
    #cc = costs_df.iloc[ii, 0] # total cost
    der_cc = costs_df.iloc[ii, 1] # mean of the derivative of the total cost
    Z += k_sim(xj, xii, W)
    #dc = (cc + der_cc)
    #kk =(k_sim(xj, xii, W))
    #E1 += kk * dc
    #E1 += (k_sim(xj, xii, W)) * (cc+der_cc)
    E1 += (k_sim(xj, xii, W)) * (der_cc)
    if ((ii+1)!=n_samples):
        xjj = xj_df.iloc[ii+1]
        if(np.sqrt(np.sum(np.square(np.array(xj) - np.array(xjj)))) > 0.001):
            # the next row contains a different sample of the dataset
...
def jac_NNopt_loss_function_err(X_df,costs_df,W):
    """
    ...
    for ii in range(n_samples):
        ...
        dE1 += cc*(-k_sim(xj, xii, W)*xdiff2*W)
        dE2 += cc*(k_sim(xj,xii,W))
        #dE = dE + cc*(-Z*k_sim(xj, xii, W)*xiidiff2*W-k_sim(xj,xii,W)*dZ)
        if ((ii + 1) != n_samples):
            xjj = xj_df.iloc[ii + 1]
            if (np.sqrt(np.sum(np.square(np.array(xj) - np.array(xjj)))) > 0.001):
                # the next row contains a different sample of the dataset
        ...
        #xiidiff2 = np.array(np.square(xj - xii),dtype=float)
        #dE = dE + cc*(1/Z**2)*(-Z*k_sim(xj, xii, W)*xiidiff2*W-k_sim(xj,xii,W)*dZ)
    #print(n_Dx)
    #print("dE: {}".format(dE/n_Dx))
    return (dE/n_Dx)

def jac_NNopt_loss_function_err_sigma(X_df,costs_df,n_D,W,r):
    """
    ...
    for jj in range(n_samples):
        ...
        #sigma = r * (dm_sorted_df.iloc[0, 0])
        Z += k_sim_sigma(xj, xi, W,sigma)
        xdiff2 = np.square(np.array(xj) - np.array(xi))
        dji2 = np.sum(xdiff2*W2)
        dZw += -k_sim_sigma(xj, xi, W,sigma) * W * (1/(sigma**2)) *(xdiff2 - ((r*dji2)/(mn*sigma)) * np.sum((dm_sorted_df.iloc[0:mn,1]/dm_sorted_df.iloc[0:mn,0])))
        #dZw += -k_sim_sigma(xj, xi, W, sigma) * W * (1 / (sigma ** 2)) * (xdiff2 - ((r * dji2) / (sigma)) * ((dm_sorted_df.iloc[0, 1] / dm_sorted_df.iloc[0, 0])))
        dE2 += cc * (k_sim_sigma(xj, xi, W, sigma))
        dE1w += cc * (-k_sim_sigma(xj, xi, W,sigma) * W * (1/(sigma**2)) *(xdiff2 - ((r*dji2)/(mn*sigma)) * np.sum((dm_sorted_df.iloc[0:mn,1]/dm_sorted_df.iloc[0:mn,0]))))
        #dE1w += cc * (-k_sim_sigma(xj, xi, W, sigma) * W * (1 / (sigma ** 2)) * (xdiff2 - ((r * dji2) / (sigma)) * ((dm_sorted_df.iloc[0, 1] / dm_sorted_df.iloc[0, 0]))))
        dZr += (k_sim_sigma(xj, xi, W,sigma)*dji2)/(r*(sigma**2))
        dE1r += cc * ((k_sim_sigma(xj, xi, W,sigma)*dji2)/(r*(sigma**2)))
        if ((jj + 1) != n_samples):
            xjj = xj_df.iloc[jj + 1]
            if (np.sqrt(np.sum(np.square(np.array(xj) - np.array(xjj)))) > 0.001):
                # the next row contains a different sample of the dataset
                ...
                if ((Z**2)<1e-60):
                    ...
                    reg = (1 / (Z ** 2))
                dEw += reg * (Z * dE1w - dE2 * dZw)
                dEr += reg * (Z * dE1r - dE2 * dZr)
                Z = 0
                dZw = 0
                dZr = 0
                dE1w = 0
    ...
    #print(n_Dx)
    #print("dE: {}".format(dE/n_Dx))
    dE = np.append((dEw/n_Dx),(dEr/n_Dx))
    return dE
    #return (dEw/n_Dx)

def jac_NNopt_loss_function_err_sigma_1(X_df,costs_df,n_D,M,W,r):
    """
    ...
    for jj in range(n_Dx):
        ...
        for ii in range(n_D):
            ...
            xdiff2 = np.square(np.array(xj) - np.array(xii))
            dji2 = np.sum(xdiff2*W2)
            #dZw += -k_sim_sigma(xj, xii, W,sigma) * W * (1/(sigma**2)) *(xdiff2 - ((r*dji2)/(mn*sigma)) * np.sum((dm_sorted_df.iloc[0:mn,1]/dm_sorted_df.iloc[0:mn,0])))
            dZw += -k_sim_sigma(xj, xii, W, sigma) * W * (1 / (sigma ** 2)) * (xdiff2 - (((r**2) * dji2) / ((mm**2) * (sigma**2))) * np.sum(dm_sorted_df.iloc[0:mm, 1]))
            #dZr += (k_sim_sigma(xj, xii, W, sigma) * dji2) / (r * (sigma ** 2))
            dZr += (k_sim_sigma(xj, xii, W, sigma) * dji2) / (r * (dm_mean ** 2))
        ...
        for ii in range(n_D):
            ...
            #dE1w += Z * (-k_sim_sigma(xj, xii, W, sigma) * W * (1 / (sigma ** 2)) * (xdiff2 - ((r * dji2) / (mn * sigma)) * np.sum((dm_sorted_df.iloc[0:mn, 1] / dm_sorted_df.iloc[0:mn, 0]))))
            dE1w = Z * (-k_sim_sigma(xj, xii, W, sigma) * W * (1 / (sigma ** 2)) * (xdiff2 - (((r**2) * dji2) / ((mm**2) * (sigma**2))) * np.sum(dm_sorted_df.iloc[0:mm, 1])))
            ...
            dEw += cc * reg * (dE1w - dE2w)
            dEr += cc * reg * (dE1r - dE2r)
...
def jac_NNopt_loss_function_err_sigma_2(X_df,costs_df,n_D,M,W):
    """
    ...
    for jj in range(n_Dx):
        ...
        for ii in range(n_D):
            ...
            dji2 = np.sum(xdiff2*W2)
            k_sim_vect.append(k_sim_sigma(xj, xii, W, sigma))
            dk_sim_vect.append(-k_sim_sigma(xj, xii, W, sigma) * W * (1 / (sigma ** 2)) * (xdiff2 - (((r**2) * dji2) / ((mm**2) * (sigma**2))) * np.sum(dm_sorted_df.iloc[0:mm, 1])))

        k_sim_df = pd.DataFrame({'k_sim': k_sim_vect,'dk_sim': dk_sim_vect})
        k_sim_sorted_df = k_sim_df.sort_values(by=['k_sim'], ascending=False)
        k_sim_sorted_df = k_sim_sorted_df.reset_index(drop=True)
        Z = np.sum(k_sim_sorted_df.iloc[0:nn,0])
        dZw = np.sum(k_sim_sorted_df.iloc[0:nn,1])
        if ((Z ** 2) < 1e-60):
            reg = 1e+60
        else:
            reg = (1 / (Z ** 2))
        ...
        E_df = pd.DataFrame({'k_sim': k_sim_vect, 'E_value': dEw_vect})
        E_sorted_df = E_df.sort_values(by=['k_sim'], ascending=False)
        E_sorted_df = E_sorted_df.reset_index(drop=True)
        dEw = np.sum(E_sorted_df.iloc[0:nn, 1])
...
def jac_NNopt_loss_function_err_sigma_3(X_df,costs_df,n_D,M,W,r):
    """
    ...
    for jj in range(n_Dx):
        ...
        for ii in range(n_D):
            ...
            dji2 = np.sum(xdiff2*W2)
            k_sim_vect.append(k_sim_sigma(xj, xii, W, sigma))
            dk_sim_w_vect.append(-k_sim_sigma(xj, xii, W, sigma) * W * (1 / (sigma ** 2)) * (xdiff2 - (((r**2) * dji2) / ((mm**2) * (sigma**2))) * np.sum(dm_sorted_df.iloc[0:mm, 1])))
            dk_sim_r_vect.append((k_sim_sigma(xj, xii, W, sigma) * dji2) / (r * (dm_mean ** 2)))

        k_sim_df = pd.DataFrame({'k_sim': k_sim_vect,'dk_sim_w': dk_sim_w_vect,'dk_sim_r': dk_sim_r_vect})
        k_sim_sorted_df = k_sim_df.sort(['k_sim'], ascending=False)
        k_sim_sorted_df = k_sim_sorted_df.reset_index(drop=True)
        Z = np.sum(k_sim_sorted_df.iloc[0:nn,0])
        dZw = np.sum(k_sim_sorted_df.iloc[0:nn,1])
        dZr = np.sum(k_sim_sorted_df.iloc[0:nn,2])
        if ((Z ** 2) < 1e-60):
            reg = 1e+60
        else:
            reg = (1 / (Z ** 2))
        ...
        E_sorted_df = E_sorted_df.reset_index(drop=True)
        dEw = np.sum(E_sorted_df.iloc[0:nn, 1])
        dEr = np.sum(E_sorted_df.iloc[0:nn, 2])
...
def jac_NNopt_loss_function_der_err_sigma_3(X_df, costs_df, n_D, M, W, r):
  """
...
def jac_NNopt_loss_function_iter_sigma_3(X_df, costs_df, n_D, M, W, r):
  """
...
class VSMModel:
  ...
  def __init__(self, X=None, Y=None, n_D=1,weights_init=None, nn=10, mm=5,r_init=0.6,reg_w=0.0001,reg_r=1):
    self.reg_w = reg_w
    self.reg_r = reg_r
    self.loss_function = NNopt_loss_function_err_sigma_3
    self.jac_loss_function = jac_NNopt_loss_function_err_sigma_3
    ...
    self.nn = nn # considered number of nearest neighbours
  ...
  def regularized_loss(self,params):
      ...
      #loss_err = self.d * self.loss_function_err(self.X, self.Y, self.n_D, self.M,self.weights)
      #loss_der_err = (1 - self.d) * self.gamma * self.loss_function_der_err(self.X, self.Y,self.n_D,self.M,self.weights,self.r)
      #loss_tot = loss_err + loss_der_err
      #loss_tot = loss_err
      #if self.online:
      # S regularization
      loss_tot_reg = loss_tot + (self.reg_w ** 2) * np.sum(np.square(np.log(np.square(self.weights / self.weights_init)))) + (self.reg_r ** 2) * np.square(np.log(np.square(self.r / self.r_init)))
  ...
  def jac_regularized_loss(self,params):
      ...
      # S regularization
      der_loss_tot_reg = der_loss_tot[:-1] + 4 * (1/self.weights) * (self.reg_w**2) * np.log(np.square(self.weights/self.weights_init))
  ...
  def fit(self,maxiter=250,disp=False):
    ...
    if type(self.weights_init) == type(None):
        self.weights_init = 0.0001 * np.ones(round(len(self.X.iloc[0,:])/2))
    ...
    print("Training...")
    #results = minimize(self.l1_regularized_loss, params, method='BFGS', options={'maxiter': maxiter, 'gtol': 1e-04, 'disp': disp})
    #results = minimize(self.l1_regularized_loss, params, method='SLSQP', options={'maxiter': maxiter, 'ftol': 1e-06, 'disp': disp})
    #print(results)
    #results = minimize(self.l1_regularized_loss, params, jac=self.jac_l1_regularized_loss,method='L-BFGS-B', options={'maxiter': maxiter,'gtol': 1e-06,'disp' : disp})
    results = minimize(self.regularized_loss, params, jac=self.jac_regularized_loss, method='SLSQP', options={'maxiter': maxiter, 'ftol':1e-06,'disp': disp})
  ...
  def predict(self,id_new,x,cold_data_in,warm_data_out):
      ...
      sigma = r * (1 / mm) * np.sum(dd_sorted_df.iloc[0:mm,0])
      ...
      nn_id = sims_sorted_df.iloc[0:nn,0] # indexes of the nn nearest neighbours
      nn_sims = sims_sorted_df.iloc[0:nn,1] # similarities of the nn nearest neighbours
